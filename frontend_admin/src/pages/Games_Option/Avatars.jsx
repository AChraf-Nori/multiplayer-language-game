...
const Avatars = () => {
  ...
  const getData = () => {
    axios
      .get("http://127.0.0.1:8000/api/Avatars")
      .then((response) => {
  ...
  const deleteLanguage = () => {
    axios.delete(`http://127.0.0.1:8000/api/Avatars/${iddelete}/delete`)
      .then((res) => {
        // if delete Succes
        setIsdelete(0)
        setiddelete(0)
  ...
  const [imageprice, setImageImageprice] = useState(0);
  ...
  const handleImagePrice = (e) => {
    setImageImageprice(e.target.value);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    axios.post("http://127.0.0.1:8000/api/Avatars", formData)
      .then((res) => {
        // if add admin Succes 
        ...
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div className="p-4 md:p-5">
                <form className="space-y-4" action="#" onSubmit={handleSubmit}>
                    <div>
                        <label for="email" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Avatar Name</label>
                        <input type="text" onChange={handleImageNameChange} value={imageName} name="Av_Name" id="Av_Name" className="outline-none bg-gray-100 border border-blue-300 text-gray-900 text-sm rounded-lg focus:border-blue-600 block w-full p-2.5" required />
                        {nameAlredyTaken ? (<span className='text-red-500 text-[14px]'>this name allredy exist!</span>) : ("")}
                    </div>
                    <div>
                        <label for="password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Avatar Price</label>
                        <input type="number" onChange={handleImagePrice} value={imageprice} name="Av_Price" id="Av_Price" className="outline-none bg-gray-50 border border-blue-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required />
                    </div>
                    <div>
                        <label for="password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Avatar Picture</label>
                        <input type="file" onChange={handleImageFileChange} class="w-full text-sm cursor-pointer rounded-lg border border-blue-300 bg-transparent outline-none transition file:mr-5 file:border-collapse file:cursor-pointer file:border-0 file:border-r file:border-solid file:border-stroke file:bg-whiter file:py-3 file:px-5 file:hover:bg-primary file:hover:bg-opacity-10 focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:file:border-form-strokedark dark:file:bg-white/30 dark:file:text-white dark:focus:border-primary" required />
                    </div>
          ...
          <div className="max-w-full overflow-x-auto">
            <table className="w-full table-auto">
              ...
              <tbody>
                {loading ? (
                ...
                ) : data.length > 0 ? (dataPag.map((avatar, key) => (
                  <tr key={key} className='text-center'>
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 flex items-center justify-center">
                        <img className='w-[50px] h-[50px] shadow-lg' src={`http://127.0.0.1:8000/avatars/${avatar.avatar}`}  alt="profilePictur" />
                    </td>
                    <td className="border-b border-[#eee] py-5 px-4">
                      <p className="text-black dark:text-white">
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 flex items-center justify-center">
                      <div className="flex items-center space-x-3.5">
                        <button onClick={(e) => deletemodale(e, avatar.id)} className="hover:text-primary">
                          <svg
